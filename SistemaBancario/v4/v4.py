...
class ContaCorrente(Conta):
    ...
    def __repr__(self):
        return f"<{self.__class__.__name__}: (agência: '{self.agencia}', número da conta: '{self.numero}', nome do cliente: '{self.cliente.nome}')>"
...
class PessoaFisica(Cliente):
    ...
    def __repr__(self) -> str:
        return f"<{self.__class__.__name__}: (data nascimento: '{self.data_nascimento}', cpf: '{self.cpf}')>"
...
class Saque(Transacao):
    ...
    def __repr__(self):
        return f"<{self.__class__.__name__}: (valor: '{self._valor}')>"
...
def log_transacao(func):
    ...
    def envelope(*args, **kwargs):
        ...
        try:
            with open(ROOTH_PATH / 'log.txt', 'a', newline= '', encoding= 'utf-8') as arquivo:
                arquivo.write(f'Data da operação: {hora_atual}\nFunção executada: {func.__name__}\nArgumentos da função: {args} e {kwargs}\nValores retornados: {resultado}\n\n')
            return resultado
